...
Copilot Workspace can be used with many existing very large repositories. However some repositories are so large that even listing their files can be a challenging exercise. Further some repositories contain highly unusual content: files with enormously long lines, or binary files with unusual extensions and so on.

Large repositories are also challenging to content selection and planing.
